...
def binaryDescend(nums):
    ...
    while mid != right and mid != left:
        ...
        fuel_mid = findFuel(nums, mid)
        fuel_left = findFuel(nums, mid - 1)
        fuel_right = findFuel(nums, mid + 1)

        if fuel_mid <= fuel_left and fuel_mid <= fuel_right:
            return fuel_mid, mid
        elif fuel_mid > fuel_left:
            right = mid
        else:
            left = mid


def decend(nums, curr, target, dir):
    found_min = False
    cursor = curr
    best = target
    # do while fuel doesn't improve
    while not found_min:
        next = cursor
        # Don't try dupes
        # while nums[cursor] == nums[next]:
        next += dir

        # Find the fuel
        fuel = findFuel(nums, nums[next])
        # if
        if best < fuel:
            found_min = True
        else:
...
def findMin(nums):
    ...
    med_fuel = findFuel(nums, nums[med])
    left = med - 1
    while nums[left] == nums[med]:
        left -= 1

    left_fuel = findFuel(nums, nums[left])

    right = med + 1
    while nums[right] == nums[med]:
        right += 1
    right_fuel = findFuel(nums, nums[right])

    print(left_fuel, med_fuel, right_fuel)
    if med_fuel < left_fuel and med_fuel < right_fuel:
        return med_fuel, med

    elif left_fuel < med_fuel:
        return decend(nums, left, left_fuel, -1)
    # elif left_fuel < med_fuel:
    else:
        return decend(nums, right, right_fuel, +1)
...
if __name__ == '__main__':
    if isPart1:
        fuel, pos = findMin(input)

        print('The answer is:', fuel, pos)
    else:
        fuel, pos = findMin2(input)
